...
public abstract class ConstantPool<T extends Constant<T>> {
  ...
  /**
   * Returns the {@link Constant} which is assigned to the specified {@code name}. If there's no
   * such {@link Constant}, a new one will be created and returned. Once created, the subsequent
   * calls with the same {@code name} will always return the previously created one (i.e.
   * singleton.)
   *
  ...
  /**
   * Creates a new {@link Constant} for the given {@code name} or fail with an {@link
   * IllegalArgumentException} if a {@link Constant} for the given {@code name} exists.
   */
