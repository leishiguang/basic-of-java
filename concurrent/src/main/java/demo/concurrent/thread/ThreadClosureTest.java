...
class ThreadClosureTest {
    ...
    void test2() throws InterruptedException {
        ...
        param1.set("主线程1");
        param2.set("主线程2");
        System.out.println(String.format("主线程，执行前：%s，%s",param1.get(),param2.get()));
        ThreadPoolUtils.getInstance().getExecutorService().execute(()->{
            System.out.println(String.format("子线程，执行前：%s，%s",param1.get(),param2.get()));
            assertNull(param1.get());
            assertNull(param2.get());
            param1.set("子线程1");
            param2.set("子线程2");
            System.out.println(String.format("子线程，执行后：%s，%s",param1.get(),param2.get()));
            assertEquals("子线程1",param1.get());
            assertEquals("子线程2",param2.get());
        });
        Thread.sleep(1000L);
        System.out.println(String.format("主线程，执行后：%s，%s",param1.get(),param2.get()));
        assertEquals("主线程1",param1.get());
        assertEquals("主线程2",param2.get());
    }
}
